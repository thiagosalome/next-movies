...
const Layout = ({ activeCategory, children }: LayoutProps) => {
  ...
  return (
    <>
      <main className={`xl:flex min-h-screen overflow-hidden ${theme}`}>
        <Sidebar showSidebar={showSidebar} categories={categories} activeCategory={activeCategory} />
        <div className={`${showSidebar ? 'left-64' : 'left-0'} relative min-h-screen transition-left duration-500 xl:flex-grow dark:bg-black`}>
