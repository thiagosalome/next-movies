...
// Types
import { FullMediaMovie } from 'types/MediaMovie'

type MovieProps = {
  media: FullMediaMovie,
  language: {
    iso_639_1: string,
    english_name: string,
    name: string
    ...
    <Layout activeCategory={query.category as string}>
      ...
      <section className="bg-gray-100 rounded-tl-3xl h-dashboard-content overflow-y-auto dark:bg-gray-800 xl:h-xl:dashboard-content">
        <div id="banner" className="relative px-4 py-5 xl:px-9">
          <Image layout='fill' src={`https://image.tmdb.org/t/p/original/${media.backdrop_path}`} className="absolute top-0 left-0 h-full w-full object-cover" alt={media.title} title={media.title} />
...
export const getStaticProps: GetStaticProps = async ({ params: { movie_id } }) => {
  const media: FullMediaMovie = await getMedia('movie', movie_id)
